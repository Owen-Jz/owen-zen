  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
      ...
      >
        ...
        <div className="text-xs text-gray-500">{formatCurrency(stats.totalYearly)}/year</div>
      </motion.div>
      ...
      >
        ...
        <div className="text-xs text-gray-500">
          {stats.mostExpensive ? formatCurrency(getMonthlyAmount(stats.mostExpensive.amount, stats.mostExpensive.billingCycle)) + "/mo" : "No data"}
        </div>
      </motion.div>
      ...
      <div className="p-4">
        ...
        <div className="mb-3">
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold">{formatCurrency(monthlyAmount)}</span>
            <span className="text-xs text-gray-500">/mo</span>
          </div>
          <div className="text-xs text-gray-500">
            {formatCurrency(yearlyAmount)}/year · {subscription.billingCycle}
          </div>
          {hasPriceChange && latestHistory && latestHistory.amount !== subscription.amount && (
            <div className="flex items-center gap-1 mt-1">
              <span className="text-xs line-through text-gray-600">
                {formatCurrency(getMonthlyAmount(latestHistory.amount, subscription.billingCycle))}/mo
              </span>
              <span className="text-xs text-green-400 flex items-center gap-0.5">
                <TrendingUp size={10} /> {formatCurrency(monthlyAmount - getMonthlyAmount(latestHistory.amount, subscription.billingCycle))}
              </span>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mt-1.5">
            <span className="text-sm text-gray-400">
              <span className="font-semibold text-white">{formatCurrency(totalMonthly)}</span>/month
            </span>
            <span className="text-sm text-gray-500">
              <span className="font-semibold">{formatCurrency(totalYearly)}</span>/year
            </span>
          </div>
        </div>
