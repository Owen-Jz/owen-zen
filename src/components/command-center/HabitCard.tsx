...
function MiniRing({ percent, size = 48 }: { percent: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} className="transform -rotate-90">
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--cc-border)" strokeWidth="5" />
      <circle
        cx={size / 2} cy={size / 2} r={r} fill="none"
        stroke="var(--cc-accent)" strokeWidth="5"
...
export function HabitCard({ todayPercent = 0, weeklyData = [], streak = 0 }: HabitCardProps) {
  return (
    <div className="rounded-xl border p-5 h-full min-h-[140px] hover:shadow-md transition-all duration-200 hover:-translate-y-px cursor-pointer bg-[var(--cc-card)] border-[var(--cc-border)]">
      <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "var(--cc-text-secondary)" }}>Habits</p>
      <div className="flex items-center justify-between">
        <div className="flex flex-col items-center">
          <MiniRing percent={todayPercent} size={56} />
          <span className="text-xs mt-1 font-semibold" style={{ color: "var(--cc-text)" }}>{Math.round(todayPercent)}%</span>
          <span className="text-[10px]" style={{ color: "var(--cc-text-secondary)" }}>today</span>
        </div>
        <div className="flex-1 ml-4">
          {weeklyData.length > 0 && (
            <div className="mb-2">
              <span className="text-[10px] uppercase tracking-wider" style={{ color: "var(--cc-text-secondary)" }}>7-day trend</span>
